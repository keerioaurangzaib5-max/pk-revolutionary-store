...
export default function RevolutionCheckout() {
  ...
  return (
    <div className="min-h-screen bg-[#050506] text-neutral-100 font-sans antialiased selection:bg-neutral-800 flex flex-col justify-between pt-20">
      ...
      {/* Main Container */}
      <div className="w-full flex-grow flex justify-center py-12 px-6 md:px-12">
        <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-12 gap-10">
          ...
          <div className="md:col-span-7 space-y-8 text-left animate-fade-in-up">
            <div>
              <h1 className="text-xl font-bold tracking-wider uppercase text-white font-sans">Secure Order Setup</h1>
              <p className="text-[10px] text-zinc-500 font-mono tracking-widest mt-1">Transaction Node // Operational</p>
            </div>

            {/* AI Voice Assist Box */}
            <div className="bg-[#ff5500]/5 border border-[#ff5500]/15 rounded-2xl p-6 space-y-4 relative overflow-hidden">
              ...
              <button 
                ...
                className={`w-full py-4 rounded-full font-sans font-bold tracking-widest uppercase text-[10px] border transition-all duration-300 flex items-center justify-center gap-2 ${
                  isRecordingVoice 
              ...
              {voiceTranscript && (
                <div className="bg-black border border-zinc-900 rounded-xl p-4 text-[11px] text-zinc-400 italic text-left font-mono">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-6">
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className="w-full bg-[#0c0d0f] border border-zinc-900 focus:border-[#ff5500] focus:shadow-[0_0_15px_rgba(255,85,0,0.1)] rounded-full px-5 py-4 text-xs text-white transition outline-none"
                />
              </div>
                ...
                <div className="col-span-1 space-y-1.5">
                  ...
                  <div className="relative">
                    <select 
                      ...
                      className="w-full bg-[#0c0d0f] border border-zinc-900 focus:border-[#ff5500] rounded-full px-4 py-4 text-xs text-white transition outline-none appearance-none cursor-pointer"
                    >
                    ...
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-zinc-500 text-[10px]">▼</span>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    ...
                    className={`py-4 rounded-full text-[10px] font-sans font-bold uppercase border text-center transition-all ${
                      paymentMethod === 'COD' 
              ...
              {feedbackMessage && (
                <div className={`p-4 rounded-2xl text-xs font-mono border text-left ${
              ...
              <button
                ...
                className="w-full bg-[#ff5500] text-black disabled:bg-zinc-900 disabled:text-zinc-600 disabled:border-transparent border border-[#ff5500] font-sans font-bold text-xs uppercase tracking-widest py-4.5 rounded-full shadow-xl transition active:scale-[0.99] kinetic-hover-btn"
              >
          ...
          <div className="md:col-span-5 space-y-6 text-left animate-fade-in-up animation-delay-100">
            <div className="bg-[#0c0d0f] border border-zinc-900 rounded-2xl p-6 space-y-6 shadow-2xl sticky top-28">
              <div>
                <h2 className="text-xs font-bold tracking-widest text-zinc-400 uppercase font-sans">Selected Gear</h2>
                <p className="text-[9px] text-zinc-600 font-mono uppercase tracking-wider mt-1">Asset Allocation List</p>
              </div>
              ...
              <div className="space-y-4 max-h-80 overflow-y-auto pr-1">
                {cart.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-[11px] font-sans text-zinc-500">No assets selected.</p>
                  ...
                  cart.map((item) => (
                    <div key={item.id} className="flex gap-4 items-center border-b border-zinc-900/40 pb-4">
                      <div className="w-12 aspect-[3/4] overflow-hidden bg-black border border-zinc-900 rounded-lg flex-shrink-0">
                        <img src={item.image} alt={item.name} className="w-full h-full object-cover grayscale" />
                      </div>
                      <div className="flex-grow text-left">
                        <h3 className="text-[11px] font-semibold tracking-wide text-zinc-300 font-sans">{item.name}</h3>
              ...
              {cart.length > 0 && (
                <div className="border-t border-zinc-900 pt-4 space-y-3 font-mono text-xs">
                  ...
                  <div className="flex justify-between items-center border-t border-zinc-900 pt-3 text-white">
                    <span className="text-[10px] uppercase tracking-wider font-sans">Estimated Total</span>
                    <span className="font-bold text-sm text-[#ff5500] font-mono">${cartTotal.toFixed(2)}</span>
                  </div>
                </div>
              )}
            </div>
      ...
      {/* Footer */}
      <footer className="p-6 text-center border-t border-zinc-900 bg-black text-[9px] uppercase tracking-[0.2em] text-zinc-600 mt-12 font-mono">
