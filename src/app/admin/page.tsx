...
export default function AdminControlRoom() {
  ...
  return (
    <div className="min-h-screen bg-[#050506] text-neutral-100 font-sans antialiased selection:bg-neutral-800 flex flex-col justify-between pt-24">
      
      {/* Admin Top Dashboard Bar */}
      <nav className="border-b border-zinc-900/50 bg-[#0d0e11]/80 backdrop-blur-xl px-6 md:px-12 py-5 flex justify-between items-center fixed top-0 left-0 right-0 z-40">
        <div className="text-left">
          <Link href="/">
            <h1 className="text-[13px] font-bold tracking-[0.25em] text-white uppercase hover:text-[#ff5500] transition font-sans">
              PREPGENIUS STUDIOS // <span className="text-[#ff5500] font-bold">HQ</span>
            </h1>
          </Link>
          <p className="text-[8px] text-zinc-500 uppercase tracking-widest font-mono mt-1">Control Center Node</p>
        </div>
      ...
      <main className="flex-grow w-full flex justify-center py-10 px-4 md:px-8">
        <div className="w-full max-w-5xl space-y-8 animate-fade-in-up">
          ...
          {/* Key Metric Panels (Dribbble Style) */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
            <div className="bg-[#0c0d0f] border border-zinc-900 rounded-2xl p-6 hover:border-zinc-800 transition duration-300">
              <span className="text-[9px] font-mono uppercase tracking-widest text-zinc-500 block">Total Orders</span>
              <p className="text-2xl font-bold text-white tracking-tight mt-1">{orders.length}</p>
            </div>
            <div className="bg-[#0c0d0f] border border-zinc-900 rounded-2xl p-6 hover:border-zinc-800 transition duration-300">
              <span className="text-[9px] font-mono uppercase tracking-widest text-zinc-500 block">COD Audits Pending</span>
              <p className="text-2xl font-bold text-[#ff5500] tracking-tight mt-1">
            ...
            <div className="bg-[#0c0d0f] border border-zinc-900 rounded-2xl p-6 hover:border-zinc-800 transition duration-300">
              ...
              <p className="text-2xl font-bold text-[#00ffd5] tracking-tight mt-1">100% OPERATIONAL</p>
            </div>
          </div>

          {/* Master Orders Table Queue */}
          <div className="bg-[#0c0d0f] border border-zinc-900 rounded-2xl overflow-hidden shadow-2xl">
            <div className="px-6 py-5 border-b border-zinc-900 bg-[#0d0e11] flex justify-between items-center">
              <h2 className="text-xs font-semibold tracking-wider text-zinc-400 uppercase font-sans">Active Distribution Pipeline</h2>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-zinc-900 text-[9px] font-mono font-bold uppercase tracking-widest text-zinc-500 bg-[#0d0e11]/20">
                    ...
                    <th className="px-6 py-4 font-sans">Customer</th>
                    <th className="px-6 py-4 font-sans">Destination</th>
                    <th className="px-6 py-4 font-sans">Amount / Mode</th>
                  ...
                  {orders.map((order) => (
                    <tr key={order.orderId} className="hover:bg-zinc-900/10 transition-colors">
                      ...
                      <td className="px-6 py-5 text-left">
                        <p className="font-semibold text-zinc-200 font-sans">{order.customerName}</p>
                        <p className="text-zinc-500 font-mono text-[10px] mt-0.5">{order.phone}</p>
                      </td>
                      <td className="px-6 py-5 text-zinc-400 text-left font-sans">
                      ...
                      <td className="px-6 py-5 text-left">
                        ...
                        <span className={`inline-block text-[8px] font-mono uppercase font-bold tracking-widest px-2.5 py-0.5 rounded-full mt-1.5 border ${
                      ...
                      <td className="px-6 py-5 text-left font-sans">
                        ...
                        {order.voiceTranscript && (
                          <div className="mt-2 text-[10px] text-zinc-500 bg-black p-3.5 rounded-xl border border-zinc-900 max-w-xs italic font-mono text-left leading-relaxed">
                      ...
                      <td className="px-6 py-5 text-right">
                        {order.status === "Pending Verification" ? (
                          <button
                            ...
                            className="bg-[#ff5500] text-black border border-[#ff5500] hover:bg-transparent hover:text-[#ff5500] text-[9px] font-sans font-bold uppercase tracking-widest px-4 py-2 rounded-full transition kinetic-hover-btn"
                          >
                            Verify COD
                          </button>
                        ) : (
